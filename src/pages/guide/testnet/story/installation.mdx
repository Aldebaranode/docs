...
STORY_NAME="geth-${OS_TYPE}-${ARCH_TYPE}"
STORY_DOWNLOAD_URL="https://github.com/piplabs/story/releases/download/v0.12.0/${STORY_NAME}"
...
GETH_DOWNLOAD_URL="https://github.com/piplabs/story-geth/releases/download/v0.10.0/${GETH_NAME}"
...
Use `story init` to set up your node for the Odyssey testnet. Replace `'Your Node Name'` with a unique identifier.
```bash copy
CHAIN_ID='odyssey'
...
[Service]
User=$USER
ExecStart=$(which story-geth) --odyssey --syncmode full --datadir $HOME/.story/geth/odyssey
Restart=always
RestartSec=3
LimitNOFILE=infinity
LimitNPROC=infinity
...
```bash copy
PERSISTENT_PEERS=$(curl "https://scan.aldebaranode.xyz/api/scan/peers?rpc_url=https://rpc-story-testnet.a
ldebaranode.xyz" | jq -r 'flatten | join(",")')
